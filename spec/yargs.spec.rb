...
describe "Yargs" do
  describe "#flag(key) | #flag?(key) {}" do
    ...
    it "removes the flag from the remaining args." do
      ...
      unless yargs.flag('flags-are-consumed') && yargs.option('flags-are-consumed').nil?
  ...
  describe "#option" do
    it "Accepts = seprated key value pairs (--value=1)" do
      ...
      expect(yargs.option('simplevalue')).to eql('1')
    end
    ...
    it "Reads all characters after the first = as the value (--value=test=ing)" do
      ...
      expect(yargs.option('longvalue')).to eql('make sure WE are R3ading = @ll (harcacter5')
    end
    ...
    it "Doesn't match another value with a common prefix" do
      ...
      fail('Matched by prefix') if yargs.option('t')
